    ...
    (async () => {
      ...
      const totals = nodes.map((node) => ({
        ...
        amount: transactions.reduce((total, transaction) => {
          if (node.id === transaction.from && node.id === transaction.to) { return total; }
          if (node.id === transaction.from) { return total - transaction.amount; }
          if (node.id === transaction.to) { return total + transaction.amount; }
          return total;
        }, 0),
      }))
